    ...
    pub async fn set_cache_up_to_step(
        ...
        if let Some(generation_prefix) = generation_prefix {
            base_req
                .instruct_prompt
                .prompt
                .build_with_generation_prefix(generation_prefix)?;
        } else {
            base_req.instruct_prompt.prompt.build()?;
        }
        base_req.set_cache_request(true).await
    }
...
impl InferenceStep {
    async fn run(
        ...
        base_req.set_base_req_stop_sequences(
            ...
            &self.step_config.stop_word_null_result,
        );
        // Request grammar
        if let Some(stop_word_null_result) = &self.step_config.stop_word_null_result {
            self.step_config
                .grammar
                .set_stop_word_null_result(stop_word_null_result);
        }
        self.step_config
            .grammar
        ...
        base_req.cascade_request(self).await
    }

    // step_counter + step_prefix + prefix_delimiter + (llm_content | stop_word_null_result) + dynamic_suffix
    fn display_outcome(&self) -> Result<String> {
        let llm_content = if let Some(llm_content) = &self.llm_content {
            llm_content
        } else if let Some(stop_word_null_result) = &self.step_config.stop_word_null_result {
            stop_word_null_result
        } else {
            return Err(anyhow!(
                "llm_content not yet set and stop_word_null_result not set."
            ));
        };
