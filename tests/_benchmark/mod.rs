...
pub struct LlmBenchmark {
    ...
    parse_method: Option<ParserMethod>,
    reason_step_token_count: u32,
    default_request_config: BaseRequestConfig,
...
impl LlmBenchmark {
    pub fn new() -> Self {
        LlmBenchmark {
            ...
            parse_method: None,
            reason_step_token_count: 300,
            default_request_config: BaseRequestConfig::default(),
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        for mut model in self.models.clone() {
            match model.backend {
                ...
                Backend::OpenAI => {
                    if let Some(parse_method) = &self.parse_method {
                        if parse_method == &ParserMethod::LogitBias {
                    ...
                    let mut llm_client = LlmClient::openai().from_model_id(&model_id).init()?;
                    llm_client.default_request_config = self.default_request_config.clone();
                    model.model_url = Some(llm_client.backend.get_model_url());
                    model.model_id = Some(self.run_model_benchmark(&llm_client).await?);
                }
                Backend::Anthropic => {
                    if let Some(parse_method) = &self.parse_method {
                        if parse_method == &ParserMethod::Grammar {
                            eprintln!(
                    ...
                    let mut llm_client = LlmClient::anthropic().from_model_id(&model_id).init()?;
                    llm_client.default_request_config = self.default_request_config.clone();
                    model.model_url = Some(llm_client.backend.get_model_url());
                    model.model_id = Some(self.run_model_benchmark(&llm_client).await?);
                }
    ...
    pub fn reason_step_token_count(&mut self, token_count: u32) -> &mut Self {
        self.reason_step_token_count = token_count;
        self
    }

    pub fn use_logit_bias_backend(&mut self) -> &mut Self {
        self.parse_method = Some(ParserMethod::LogitBias);
        self
    }

    pub fn use_grammar_backend(&mut self) -> &mut Self {
        self.parse_method = Some(ParserMethod::Grammar);
        self
    }

    pub fn use_basic_backend(&mut self) -> &mut Self {
        self.parse_method = Some(ParserMethod::Basic);
        self
    }
            ...
            let result_row = match question.test_type {
                ...
                _ => {
                    let decider = llm_client
                        .decider()
                        .reason_step_token_count(self.reason_step_token_count)
                        .dynamic_temperature(self.dynamic_temperature)
                        .best_of_n_votes(self.best_of_n_votes);

                    let decider = if let Some(parse_method) = &self.parse_method {
                        match parse_method {
                            ParserMethod::Basic => decider.use_basic_backend(),
                            ParserMethod::LogitBias => decider.use_logit_bias_backend(),
                            ParserMethod::Grammar => decider.use_grammar_backend(),
                        }
                    } else {
                        decider
                    };

                    let parse_method = match &decider.decision_config.parse_method {
                        ParserMethod::Basic => "Basic".to_string(),
                        ParserMethod::LogitBias => "LogitBias".to_string(),
                        ParserMethod::Grammar => "Grammar".to_string(),
                    };
                    let res = match question.test_type {
                        ...
                        TestType::TextGen => unreachable!("TextGen should not be here"),
                    };
                    let total_time = chrono::Utc::now() - start_timestamp;
                    match res {
                        Ok(res) => {
...
impl BaseRequestConfigTrait for LlmBenchmark {
