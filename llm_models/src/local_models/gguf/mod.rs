...
// Internal imports
use super::{metadata::LocalLlmMetadata, LocalLlmModel};
use crate::LlmChatTemplate;
use loaders::local::GgufLocalLoader;

// Feature-specific internal imports

use self::{
    super::hf_loader::{HfTokenTrait, HuggingFaceLoader},
    loaders::hf::GgufHfLoader,
};
#[cfg(feature = "model-tokenizers")]
use crate::{
    local_models::gguf::tools::gguf_tokenizer::convert_gguf_to_hf_tokenizer,
    tokenizer::LlmTokenizer,
};

#[derive(Default, Clone)]
pub struct GgufLoader {
    pub gguf_preset_loader: crate::gguf_presets::loader::GgufPresetLoader,
...
impl GgufLoader {
    ...
    pub fn load(&mut self) -> crate::Result<LocalLlmModel> {
        ...
        if self.gguf_hf_loader.hf_quant_file_url.is_some() {
...
impl crate::gguf_presets::GgufPresetTrait for GgufLoader {
    fn preset_loader(&mut self) -> &mut crate::gguf_presets::loader::GgufPresetLoader {
...
pub trait GgufLoaderTrait {
    ...
    fn model_id<S: AsRef<str>>(&mut self, model_id: S) -> &mut Self {
        {
            // Weird error here, maybe a bug - wrapping resolves
